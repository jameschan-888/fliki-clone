      ...
      <div className="workflow-page">
        ...
        <div style={{ maxWidth: 720, margin: "32px auto", padding: "0 24px" }}>
          ...
          <form onSubmit={submit}>
            <div style={{ marginBottom: 12 }}><label htmlFor="wf-title">标题</label><input id="wf-title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} style={{ width: "100%", padding: 8 }} /></div>
            <div style={{ marginBottom: 12 }}><label htmlFor="wf-language">语言</label><input id="wf-language" type="text" value={language} onChange={(e) => setLanguage(e.target.value)} style={{ width: 200, padding: 8 }} /></div>
            {mode === "translate" && <div style={{ display: "flex", gap: 12, marginBottom: 12 }}><div><label htmlFor="wf-source-lang">源语言</label><input id="wf-source-lang" value={sourceLang} onChange={(e) => setSourceLang(e.target.value)} style={{ padding: 8 }} /></div><div><label htmlFor="wf-target-lang">目标语言</label><input id="wf-target-lang" value={targetLang} onChange={(e) => setTargetLang(e.target.value)} style={{ padding: 8 }} /></div></div>}
            <div style={{ marginBottom: 12 }}>{mode === "slides" && <><label htmlFor="wf-pptx">PPTX 文件</label><input id="wf-pptx" type="file" accept=".pptx" onChange={(event) => setPptxFile(event.target.files?.[0] || null)} /></>}{mode === "translate" && <><label htmlFor="wf-media">媒体文件</label><input id="wf-media" type="file" accept="video/*,audio/*" onChange={(event) => setMediaFile(event.target.files?.[0] || null)} /></>}<label htmlFor="wf-input">{props.inputLabel}</label><textarea id="wf-input" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={props.inputPlaceholder} rows={mode === "url" ? 2 : 12} style={{ width: "100%", padding: 8, fontFamily: mode === "slides" ? "monospace" : "inherit" }} /></div>
            {mode === "record" && <div style={{ marginBottom: 16 }}><button type="button" aria-label={recording ? "停止录屏录音" : "开始录屏录音"} onClick={toggleRecording} style={{ padding: "8px 16px" }}>{recording ? "停止录制" : "开始录屏/录音"}</button>{recordingReady && <span style={{ marginLeft: 12, color: "#087f23" }}>录制已完成，请补充转写文本</span>}</div>}
            <button type="submit" disabled={busy || recording} aria-busy={busy} style={{ padding: "8px 24px", background: busy || recording ? "#ccc" : "#2c7be5", color: "#fff", border: "none", borderRadius: 4, cursor: busy || recording ? "wait" : "pointer" }}>{busy ? "生成中..." : "生成草稿"}</button>
          </form>
          {error && <div role="alert" style={{ marginTop: 16, padding: 12, background: "#fee", color: "#c00", borderRadius: 4 }}>错误: {error}</div>}
