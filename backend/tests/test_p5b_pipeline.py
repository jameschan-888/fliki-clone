...
class P5BPipelineTest(unittest.TestCase):
    ...
    def test_unconfirmed_draft_cannot_start_generation(self):
        ...
        connection=main.get_db();timestamp="2026-07-23T00:00:00+00:00"
        connection.execute("INSERT INTO workflow_drafts (id,title,source_script,status,version,created_at,updated_at,user_id) VALUES ('draft','t','s','draft',1,?,?,?)",(timestamp,timestamp,self.user_id));connection.commit();connection.close()
    ...
    def test_force_creates_a_new_run_after_success(self):
        ...
        connection.execute("INSERT INTO workflow_runs (id,workflow_draft_id,status,progress,created_at,updated_at,user_id) VALUES ('old-run','draft-force','success',100,?,?,?)",(timestamp,timestamp,self.user_id));connection.commit();connection.close()
