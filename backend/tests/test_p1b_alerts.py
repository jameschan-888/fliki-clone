"""R24 (rev43): P1-B 告警 webhook 测试 - TestClient 收口.
原 rev24 阶段 D P1-B. 改写原因同 test_p1a_user_metrics.py.
...
import hashlib, hmac, json, os, sys, time, unittest
...
ALERT_SECRET = "fliki-alert-dev-secret-CHANGE-IN-PROD"


def _register(client, email, password="test12345"):
...
class P1BAlertsTests(unittest.TestCase):
    "R24: 告警 webhook end-to-end via TestClient."
    ...
    def setUpClass(cls):
        ...
        cls.token = _register(cls.client, "p1b-" + str(int(time.time())) + "@e.com")["token"]

    def _h(self, token=None):
        return {"Authorization": "Bearer " + token} if token else {}

    def _post(self, path, token=None):
        return self.client.post(path, headers=self._h(token)).json()

    def _get(self, path, token=None):
        return self.client.get(path, headers=self._h(token)).json()

    def _post_raw(self, path, token=None):
        r = self.client.post(path, headers=self._h(token))
        return r.status_code, r.content

    def _get_raw(self, path, token=None):
        r = self.client.get(path, headers=self._h(token))
        return r.status_code, r.content
