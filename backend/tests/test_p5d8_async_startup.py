...
class AsyncStartupTests(unittest.TestCase):
    def test_status_initial_pending(self):
        ...
        s = main_mod._startup_diagnostic_status
    ...
    def test_background_diagnostic_runs(self):
        ...
        with patch.object(main_mod, "write_startup_diagnostic", return_value=calls["report"]):
            t0 = time.time()
            main_mod._background_diagnostic()
            dur = time.time() - t0
            self.assertLess(dur, 1.0, "_background_diagnostic should be fast under test")
        status = main_mod._startup_diagnostic_status
        self.assertEqual(status["state"], "ready")
        self.assertIsNotNone(status["finished_at"])

    def test_background_diagnostic_handles_error(self):
        with patch.object(main_mod, "write_startup_diagnostic", return_value={"error": "boom"}):
            main_mod._background_diagnostic()
        status = main_mod._startup_diagnostic_status
    ...
    def test_background_diagnostic_handles_exception(self):
        with patch.object(main_mod, "write_startup_diagnostic", side_effect=RuntimeError("crash")):
            main_mod._background_diagnostic()
        status = main_mod._startup_diagnostic_status
        self.assertEqual(status["state"], "error")
